...
//		System.out.println(LCSTD(text1,text2,0,0,dp));
	...
	public static int LCSTD(String text1,String text2,int i,int j,int[][] dp) {
		...
		int ans = 0;
		if(text1.charAt(i) == text2.charAt(j)) {
			ans = 1 + LCSTD(text1,text2,i+1,j+1,dp);//ya value hai at dp[i+1][j+1];
		}else {
			int f = LCSTD(text1,text2,i+1,j,dp);//ya value hai at dp[i+1][j];
			int s = LCSTD(text1,text2,i,j+1,dp);//ya value hai at dp[i][j+1];
			ans = Math.max(f, s);
		}
		dp[i][j] = ans;
		return ans;
