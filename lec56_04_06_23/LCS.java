...
public class LCS {
	...
	public static int lcsTD(String s1,String s2,int i,int j,int[][] dp) {
		...
		int ans = 0;
		if(s1.charAt(i) == s2.charAt(j)) {
			ans = 1+ lcsTD(s1,s2,i+1,j+1,dp);//ya dp ma dp[i+1][j+1] pa hoga
		}else {
			int f = lcsTD(s1,s2,i+1,j,dp);//ya dp ma dp[i+1][j] pa hoga
			int s = lcsTD(s1,s2,i,j+1,dp);//ya dp ma dp[i][j+1] pa hoga
			ans = Math.max(f, s);
		}
		return dp[i][j] = ans;
	}
